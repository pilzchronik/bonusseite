# DNA-Matches: Klarnamen-Mapping (intern, nicht veröffentlichen)

**Zweck:** Überblick über die auf der öffentlichen DNA-Matches-Seite verwendeten Initialen/Kurznamen und ihre zugehörigen Klarnamen. Diese Datei liegt bewusst im `_intern`-Verzeichnis der Bonusseite, das Jekyll nicht in die veröffentlichte Seite baut.

**Stand:** 2026-04-19 (Anonymisierungsrunde 2).
**Pflege:** Bei jeder Änderung der DNA-Matches-Seite aktuell halten.

## Personen, die auf der Seite mit Initialen/Kurznamen erscheinen

| Auf der Seite | Klarname | Kit | Woher bekannt |
|---|---|---|---|
| Walter Steven Z. | Walter Steven Zeisl | T405845 | UEBERGABE_fuer_Seitenprojekt.md (DNA-Projekt) |
| petctdoc | — (GEDmatch-Pseudonym, Klarname nicht veröffentlicht) | T093054 | UEBERGABE_fuer_Seitenprojekt.md |
| Reeva K. | Reeva Jacobson Kimble | T286556 | UEBERGABE_fuer_Seitenprojekt.md |
| Renee F. | Renee Fisher | M524793 | UEBERGABE_fuer_Seitenprojekt.md |
| Victor N. | Victor Neumann | T080523 | UEBERGABE_fuer_Seitenprojekt.md |
| Herbert E. F. | Herbert E. Fisher (†, Kit-Verwalterin posthum: Tochter Susan F. Boyer) | T725386 (GEDmatch) + FTDNA-Kit | Anonymisierungsrunde 2 (2026-04-19), vorher offen auf der Seite; Kit-Konstellation klargestellt 12.5.2026 |
| Lester F. | Lester Fisher (†, Halbbruder/Doppelcousin Herberts, Kit-Verwalterin posthum: Nichte Susan F. Boyer) | FTDNA (Nummer nicht bekannt) | Eingeführt 12.5.2026; Match-Status zu Wolfgang ungeprüft; doppelte Vodička-Genetik (Mütter Minnie/Elsie Vodička waren Schwestern) |
| Scott K. | Scott Kieserman | — | Anonymisierungsrunde 2 (2026-04-19), vorher offen auf der Seite |
| Joaquin R. | — im Fachprojekt DNA einsehen | BV5754921 | — |
| Sean G. | — im Fachprojekt DNA einsehen | — | — |
| Jacqueline J. | — im Fachprojekt DNA einsehen | — | — |
| Robert C. | — im Fachprojekt DNA einsehen | — | — |
| David C. | — im Fachprojekt DNA einsehen | M352745 | — |
| Gloria L. | — im Fachprojekt DNA einsehen | — | — |
| Martin S. | — im Fachprojekt DNA einsehen | — | — |
| Robert H. | — im Fachprojekt DNA einsehen | — | — |
| peter w. | — im Fachprojekt DNA einsehen | — | — |
| Sally S. S. | — im Fachprojekt DNA einsehen | — | — |
| Esther R. | — im Fachprojekt DNA einsehen | — | — |
| S. S. | — im Fachprojekt DNA einsehen | — | — |
| Susan B. | Susan F. Boyer — Kit-Verwalterin posthum für Herbert E. Fisher († Vater) und Lester Fisher († Onkel); kein eigener DNA-Test | — | Aktualisiert 12.5.2026 |
| Vivian K. | — im Fachprojekt DNA einsehen (Verwalterin für Victor N., JewishGen Hungarian SIG Director) | — | — |
| Rebecca G. | — im Fachprojekt DNA einsehen | — | — |
| Ronald Sch. | Ronald Schönberg (Sohn Arnold Schönbergs, Vater E. Randol Schönbergs) | — | Öffentlich bekannt aus Schönberg-/Zeisl-Familiengeschichte |
| Peter C. in Brünn | — im Fachprojekt DNA einsehen (Y-DNA-Match) | — | — |
| Meredith (M./Robert C.) | — im Fachprojekt DNA einsehen (steht mit Robert C. in Verbindung) | — | — |
| yaronk | — (GEDmatch-Pseudonym) | — | — |

## Personen, die auf der Seite bewusst mit vollem Namen erscheinen

| Name | Begründung |
|---|---|
| Wolfgang Pilz | Autor der Seite, Selbstvorstellung |
| Erich Zeisl (1905–1959) | Komponist, öffentliche Person, verstorben |
| Arnold Schönberg (1874–1951) | Komponist, öffentliche Person, verstorben |
| E. Randol Schönberg | Öffentliche Person (Klimt-Anwalt, JewishGen Austria-Czech SIG Mitbegründer, aktiv auf Geni.com) — Kontakt ausdrücklich öffentlich auf der Seite dokumentiert |
| Barbara (Zeisl, verh. Schönberg) | Tochter Erich Zeisls, öffentlich über die Schönberg-Familienbiografie identifizierbar; auf der Seite nur mit Vornamen |
| Wilhelm Zeisl (1907–1972), Siegmund Zeisl, Emanuel Moritz Zeisl, Egon/Walter/Erich Zeisl (Söhne), Kamilla Zeisl geb. Feitler, Malvine Feitler, Hermine Dannhauser geb. Zeisl, Josef Dannhauser, Philipine und Walter Dannhauser | Historische Familienpersonen der Zeisl-Linie, alle verstorben; zentral für die genealogische Darstellung |
| Philipp Bechinie, Theresia/Pessel, Moshe, Regina Stern, Seligmann Fürth, weitere Ahnen | Historische Vorfahren (18./19. Jh.), verstorben |

## Lücken, die nur das DNA-Fachprojekt füllen kann

Die mit „— im Fachprojekt DNA einsehen" markierten Zeilen sind aus Rücksicht auf die Rollenteilung hier nicht ergänzt. Der Klarname (falls dokumentiert) liegt im Arbeitsordner `DNA AHNENFORSCHUNG` (Statusblatt, One-to-One-Ergebnisse, Tier-1-CSV). Wolfgang kann die Lücken manuell eintragen, wenn er die Tabelle als ruhenden Überblick pflegen möchte.
